...
![Earmax Pro](images/Earmax%20Pro.jpg)
...
The official website claims it can drive 32-ohm to 600-ohm loads, with output power up to 10 mW and distortion at 0.1%. The 10 mW rating is marginal—barely enough for ordinary listening levels depending on ear sensitivity and headphone impedance. In my simulation, I cannot reach those figures unless the main supply is raised from 90 V to 200 V. 

Measurements from L7audiolab show the same trend: at a 4 V RMS output level, its FFT spectrum looks messy, the distortion is high, and the amplifier is obviously overdriven. However, 4 V RMS is actually well above typical listening levels and serves as a stress test to verify adequate power capacity. 

This explains why many users enjoy the Earmax Pro at normal listening levels, despite L7audiolab's poor measurements. 

To make this topology undoubtedly extrodinary, here I introduce a higher-power version, the Earmax Pro Max:

![Earmax Pro Max](images/Earmax%20Pro%20Max.svg)

It uses a KT88 in triode configuration, replacing the small-signal ECC88 in the original design, and raises quiescent current from about 10 mA to 100 mA. This makes the output stage strong enough to drive headphones down to 32 ohms, and distortion drops from 0.1% to 0.01% when driving 300-ohm loads at the same output level.
...
![SinglePower MPX3](images/Singlepower_mpx3.jpg)
...
The SinglePower MPX-3 was the mid-range model in its product line, selling for about $1000 at the time. It is a differential-input, cathode-follower-output amplifier with no global feedback. This kind of simple textbook circuits do not need to explain.
...
The original schematic does not worth rebuild today, I present a fine-tuned SinglePower-style headphone amplifier that combines a load-line distortion-cancellation method I learned from Nelson Pass {{#cite pass2009thesweetspot}} with gradient-descent optimization, reducing distortion from 0.5% to 0.007%, which is astonishing for an amplifier without global feedback.
